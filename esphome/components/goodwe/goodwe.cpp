...
uint16_t endian(const uint16_t &v) { return ntohs(v); }
uint32_t endian(const uint32_t &v) { return ntohl(v); }

template<typename T, T denominator> struct Data {
  T data;
  operator float() { return float(endian(data)) / float(denominator); }
